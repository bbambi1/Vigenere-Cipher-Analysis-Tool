...
def code_vigenere(message, cle, to_decode=False):
    ...
    n = len(message)
    for i in range(n):
        # On determine le decalage qu'on va appliquer a chaque lettre
...
def encode_vigenere(string):
    print("\n-------------- Encodage --------------")
    texte = open(string).read()
    cle = input("Entrez la cle :\n")
    # Tant que la ce rentree n'est pas valide
...
def decode_vigenere(string):
    message = open(string).read()
    print("\n-------------- Decodage --------------")
    ...
    if KeyKnown.upper() == "OUI":
        ...
        while (not cle.isalpha()) or (not cle.isupper()):
            print("La cle rentree n'est pas valide.\nN'utilisez que des lettres en majuscule")
            cle = input("Entrez la cle :\n")
        message_decode = decode_vigenere_withKey(message, cle)
        print("Le texte en clair est : \n", message_decode, sep="")
        return
    print("Tentative de decodage sans la cle...")
    # On determine la longueur probable de la cle
...
if sys.argv[1] == "-e":
    encode_vigenere(sys.argv[2])
elif sys.argv[1] == "-d":
    decode_vigenere(sys.argv[2])
